...
class GomokuAgent:
    ...
    Alpha–beta Gomoku agent with:
      ...
      - Intersection detection (double-4, four-3, double-3) for sharper ordering
    """
    ...
    # Intersection bonuses (used only in move ordering)
    DOUBLE_FOUR_BONUS = 400_000     # two 4s at once (very strong)
    FOUR_THREE_BONUS = 150_000      # a 4 plus a 3
    DOUBLE_THREE_BONUS = 60_000     # two 3s (threat pair)
    ...
    # -------- Intersection / Threat helpers --------
    def _line_through(self, board: np.ndarray, i: int, j: int, di: int, dj: int):
        n = BOARD_SIZE
        # walk backwards
        x, y = i, j
        while 0 <= x - di < n and 0 <= y - dj < n:
            x -= di
            y -= dj
        arr = []
        pos = None
        ...
        while 0 <= cx < n and 0 <= cy < n:
            arr.append(board[cx, cy])
            if cx == i and cy == j:
                pos = idx
            cx += di
            cy += dj
            idx += 1
        return np.array(arr, dtype=int), int(pos)

    def _scan_run(self, arr: np.ndarray, pos: int, player: int, blank: int):
        n = arr.size
        P, B = int(player), int(blank)
        L = pos
        while L - 1 >= 0 and arr[L - 1] == P:
            L -= 1
        R = pos
        while R + 1 < n and arr[R + 1] == P:
            R += 1
        run_len = R - L + 1
        left_open = (L - 1 >= 0 and arr[L - 1] == B)
        right_open = (R + 1 < n and arr[R + 1] == B)
        return run_len, left_open, right_open

    def _broken_three_count_at_pos(self, arr: np.ndarray, pos: int, player: int, blank: int) -> int:
        n = arr.size
        P, B = int(player), int(blank)
        cnt = 0
        ...
        for start in range(max(0, pos - 3), min(pos, n - 4) + 1):
            window = arr[start:start + 4]
            if np.count_nonzero(window == P) == 3 and np.count_nonzero(window == B) == 1 and arr[pos] == P:
                left_ext = cell(start - 1)
                right_ext = cell(start + 4)
                open_ends = (1 if left_ext == B else 0) + (1 if right_ext == B else 0)
                if open_ends >= 1:
                    cnt += 1
        return cnt

    def _threats_created_by_move(self, board: np.ndarray, i: int, j: int, player: int):
        B = self.blank_symbol
        if board[i, j] != B:
            return {"open4": 0, "closed4": 0, "open3": 0, "broken3": 0}

        board[i, j] = int(player)
        dirs = [(1, 0), (0, 1), (1, 1), (1, -1)]
        counts = {"open4": 0, "closed4": 0, "open3": 0, "broken3": 0}

        for di, dj in dirs:
            arr, pos = self._line_through(board, i, j, di, dj)
            run_len, left_open, right_open = self._scan_run(arr, pos, player, B)
            ...
            if run_len == 4:
                if open_ends == 2:
                    ...
                    counts["closed4"] += 1
            elif run_len == 3 and open_ends == 2:
                counts["open3"] += 1

            counts["broken3"] += self._broken_three_count_at_pos(arr, pos, player, B)

        board[i, j] = B
        return counts

    def _intersection_bonus_for_move(self, board: np.ndarray, i: int, j: int, player: int):
        c = self._threats_created_by_move(board, i, j, player)
        fours = c["open4"] + c["closed4"]
        threes = c["open3"] + c["broken3"]

        if fours >= 2:
            return self.DOUBLE_FOUR_BONUS, "double_four", c
        if fours >= 1 and threes >= 1:
            return self.FOUR_THREE_BONUS, "four_three", c
        if threes >= 2:
            return self.DOUBLE_THREE_BONUS, "double_three", c
        return 0, "", c

    def explain_intersections(self, board: np.ndarray, i: int, j: int, for_agent: bool = True):
        player = self.agent_symbol if for_agent else self.opponent_symbol
        return self._intersection_bonus_for_move(board, i, j, player)

    def _order_moves(self, board: np.ndarray, moves: List[Tuple[int, int]], maximizing: bool) -> List[Tuple[int, int]]:
        """
        Simple move ordering with intersection-aware bonuses to prefer
        double-4 / four-3 / double-3 choke moves.
        """
        scored: List[Tuple[float, Tuple[int, int]]] = []
        if maximizing:
            for (i, j) in moves:
                board[i, j] = self.agent_symbol
                base = self.evaluate_board(board)
                board[i, j] = self.blank_symbol
                bonus, _, _ = self._intersection_bonus_for_move(board, i, j, self.agent_symbol)
                scored.append((base + bonus, (i, j)))
            ...
            for (i, j) in moves:
                ...
                base = self.evaluate_board(board)
                board[i, j] = self.blank_symbol
                bonus, _, _ = self._intersection_bonus_for_move(board, i, j, self.opponent_symbol)
                scored.append((base - bonus, (i, j)))
